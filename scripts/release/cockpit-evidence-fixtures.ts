...
function success(issue: number, empty = false) {
  ...
  const body: Record<number, unknown> = {
    ...
    653: { rows: empty ? [] : [{ partyId: "party-evidence", name: "Syntetisk part", roles: ["vendor"], recentActivity: "2026-01-15", computedSpend: 3051.1999999999994, partyLink: { href: "/companies/evidence-fixture/parter/party-evidence" } }] },
...
const partyProfile = JSON.stringify({ ok: true,
  ...
  computed: {
    ...
    purchase: { amount: 3051.1999999999994, currency: "mixed" }, sales: { amount: -1234567.8, currency: "mixed" }, recentActivity: "2026-01-15",
  },
  research: {
    assertions: [{ field: "name", value: "Syntetisk part", source: "cockpit-evidence", observed_at: "2026-01-15", review_state: "approved", payload_hash: "synthetic-name-hash" }],

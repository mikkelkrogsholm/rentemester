...
// This identity is intentionally scoped to the temporary smoke company by
// passing it to `init`; no production policy is edited for the smoke test.
...
const init = Bun.spawn(["bun", CLI_PATH, "init", "--company", COMPANY, "--actor", SMOKE_MCP_ACTOR], {
  stdout: "pipe",
  stderr: "pipe",
});
await init.exited;

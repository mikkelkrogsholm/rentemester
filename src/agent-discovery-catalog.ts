...
export const AGENT_WORKFLOWS: readonly AgentWorkflow[] = [
  ...
  workflow({ id: "document-party-resolution", capabilityId: "document-party-resolution", title: "Document party resolution", intendedOutcome: "Make exactly one visible party-resolution state without changing document evidence, VAT, or journals.", steps: [
    ...
    read("coverage-plan",mcp("party_coverage_plan"),"Deduplicate exact candidates and hash-bind reviewed bank decisions, non-tax source observations, or an unresolved-external next action.",{dependsOn:["coverage"],boundary:"dry-run",canonicalRecords:["party coverage plan"]}),
    write("coverage-apply",mcp("party_coverage_apply"),"Atomically append only the exact reviewed operations, including document-hash-bound unresolved-external follow-up; leave genuinely unprocessed rows untouched.",{dependsOn:["coverage-plan"],expectedIdempotent:true,retryClass:"natural-idempotent",canonicalRecords:["document party link event","party coverage bank resolution event","party coverage batch event"]}),
...
const capabilityTuples: CapabilityTuple[] = [
  ...
  ["document-party-resolution", "Document party resolution", "Resolve a document to canonical party relations, including reviewed source-bound legacy observations, an explicit internal no-party decision, or a bounded unresolved state.", "documents", ["link document party", "review legacy document counterparty", "supplement missing vendor identity", "confirm internal voucher has no external party", "inspect document party resolution"], ["document party", "legacy identity", "source hash", "issuer", "supplier", "bank", "payer", "payment descriptor"], "company", ["document-party-resolution"]],

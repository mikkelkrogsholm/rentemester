...
export function listKnowledgeBacklinks(db: Database, input: { scope: KnowledgeScope; pageId: string; asOf?: string }) { if (!identifier.test(input.pageId)) throw new Error("invalid page id"); return listKnowledgeRelations(db, input).flatMap(relation => relation.subject.kind === "page" && relation.subject.ref === input.pageId ? [{ ...relation, direction: "outgoing" as const, label: relation.labels.forward, other: relation.object }] : relation.object.kind === "page" && relation.object.ref === input.pageId ? [{ ...relation, direction: "incoming" as const, label: relation.labels.inverse, other: relation.subject }] : []); }

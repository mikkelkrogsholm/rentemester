...
export function register(dispatch: CommandDispatch): void {
  ...
  dispatch.on("purchase-case", "group-review", async (ctx) => {
    ...
    const db=openCommandDb(ctx);try{migrate(db);const actor=audit(ctx);const principal=await authenticated(ctx,"purchase_case_group_review");const payload={groupId:ctx.arg("--group-id")??null,members:members!,expectedPolicyEventHash:ctx.arg("--expected-policy-event-hash")??null,documentationOutcome,note:ctx.arg("--note")??""};const context=approvalContext(ctx,principal,payload.expectedPolicyEventHash??undefined);try{const run=executeLocalIdempotentMutation(db,{key:validateIdempotencyKey(ctx.arg("--idempotency-key")??ctx.fatal("--idempotency-key is required")),operation:"purchase_case_group_review",principal,payload,actor,execute:()=>reviewPurchaseCaseGroup(db,{...payload,groupId:payload.groupId??undefined,actor,approval:context})});ctx.emitResult(run.receipt?{...run.result,idempotency:run.receipt}:run.result);}finally{context.controlDb.close();}}finally{db.close();}
  });
}

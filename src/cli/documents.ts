...
export function register(dispatch: CommandDispatch): void {
  ...
  dispatch.on("documents","party-coverage-plan",ctx=>{const ledger=openLedgerReadOnly(companyPaths(ctx.companyRoot()).db),control=registry(ctx);try{ctx.emitResult(planPartyCoverage(ledger,control,coverageInput(ctx)) as any);}finally{control.close();ledger.close();}});

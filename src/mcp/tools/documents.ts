...
const partyCoverageDecision=z.object({bankTransactionId:z.number().int().positive(),partyId:z.string().min(3).max(64).optional(),role:z.enum(DOCUMENT_PARTY_ROLES).optional(),noExternalParty:z.boolean().optional(),evidenceReference:z.string().min(1).max(500),rationale:z.string().min(1).max(1000)});
...
export function registerDocumentTools(server: McpServer): void {
  ...
  server.registerTool("party_coverage_plan",{title:"Plan safe party coverage completion",description:"Read-only hash-bound plan. Deduplicates exact document candidates and accepts explicit evidence-bound decisions only for reconciled rows with no source document.",inputSchema:{company:z.string().min(1),...partyCoverageFields,decisions:z.array(partyCoverageDecision).max(100).optional()},outputSchema:envelopeShape,annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false}},withCompanyReadOnlyDb<any>(({db,args})=>{const scope=documentPartyScope(args.company),registry=openWorkspaceControlReadOnlyDb(scope.workspace);try{return successEnvelope(planPartyCoverage(db,registry,{...args,companySlug:scope.companySlug}));}finally{registry.close();}}));

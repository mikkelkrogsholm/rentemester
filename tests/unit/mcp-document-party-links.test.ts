...
beforeAll(async () => {
  ...
  coverageDocumentId=coverageDocument.id;
...
describe("#588 MCP black-box contract", () => {
  ...
  test("#646 MCP keeps distinct shared-document counterparties scoped to their bank rows",async()=>{
    const db=openDb(companyPaths(companyRoot).db);const bank=db.query("SELECT id FROM bank_accounts ORDER BY id LIMIT 1").get() as {id:number};db.query("INSERT INTO bank_transactions(id,transaction_date,text,amount,currency,transaction_hash,bank_account_id,retain_until) VALUES(702,'2026-09-04','Second shared-source payment',-50,'DKK','coverage-bank-702',?,'2031-12-31')").run(bank.id);const posting=postJournalEntry(db,{transactionDate:"2026-09-04",text:"Second shared source",documentId:coverageDocumentId,sourceBankTransactionId:702,createdBy:"agent:synthetic",lines:[{accountNo:"7000",debitAmount:50},{accountNo:"2000",creditAmount:50}]});if(!posting.ok)throw new Error(posting.errors.join("; "));db.close();
    const registry=openWorkspaceControlDb(workspace);for(const party of [{id:"party-mcp-authority",role:"authority" as const},{id:"party-mcp-payee",role:"employee" as const}]){createParty(registry,{partyId:party.id,kind:party.role==="employee"?"person":"organization",name:party.id,source:"synthetic",observedAt:"2026-09-04T00:00:00.000Z",reviewAssertion:"reviewed row",actor:"user:test"});linkPartyRole(registry,{partyId:party.id,companySlug:"acme-aps",role:party.role,actor:"user:test"});}registry.close();
    const projected=(await client.send("tools/call",{name:"party_coverage",arguments:{company:"acme-aps"}})).result?.structuredContent.data;const row700=projected.rows.find((row:any)=>row.bankTransactionId===700),row702=projected.rows.find((row:any)=>row.bankTransactionId===702);const decisions=[{bankTransactionId:700,scope:"bank_transaction",transactionHash:row700.transactionHash,documentHash:row700.documentHash,partyId:"party-mcp-authority",role:"authority",provenance:"reviewed_mcp_bank_row",evidenceReference:"row 700",rationale:"authority row"},{bankTransactionId:702,scope:"bank_transaction",transactionHash:row702.transactionHash,documentHash:row702.documentHash,partyId:"party-mcp-payee",role:"employee",provenance:"reviewed_mcp_bank_row",evidenceReference:"row 702",rationale:"payee row"}];
    const planned=(await client.send("tools/call",{name:"party_coverage_plan",arguments:{company:"acme-aps",decisions}})).result?.structuredContent;expect(planned).toMatchObject({ok:true,data:{plan:{operations:[{kind:"bank_decision"},{kind:"bank_decision"}]}}});const args={company:"acme-aps",decisions,planHash:planned.data.plan.planHash,idempotencyKey:"coverage-mcp-646",confirm:true};expect((await client.send("tools/call",{name:"party_coverage_apply",arguments:args})).result?.structuredContent).toMatchObject({ok:true,data:{applied:2,idempotent:false}});expect((await client.send("tools/call",{name:"party_coverage_apply",arguments:args})).result?.structuredContent).toMatchObject({ok:true,data:{applied:2,idempotent:true}});
    const after=(await client.send("tools/call",{name:"party_coverage",arguments:{company:"acme-aps"}})).result?.structuredContent.data.rows;expect(after.find((row:any)=>row.bankTransactionId===700)).toMatchObject({candidate:{partyId:"party-mcp-authority"}});expect(after.find((row:any)=>row.bankTransactionId===702)).toMatchObject({candidate:{partyId:"party-mcp-payee"}});
  });

...
export function BookkeepingBatchView() {
  ...
  if (loading && !years) return <section data-evidence-issue="650"><h2 data-evidence-heading>Bogføringsarbejdsbord</h2><p data-evidence-status="loading">Henter bogføringskø</p></section>;
  if (error && !years) return <section data-evidence-issue="650"><h2 data-evidence-heading>Bogføringsarbejdsbord</h2><p data-evidence-status={/403|forbudt|adgang/i.test(error) ? "warning-or-blocked" : "error"}>{/403|forbudt|adgang/i.test(error) ? "Bogføring kræver afklaring" : "Bogføringskø kunne ikke hentes"}</p><p role="alert">{error}</p></section>;
  ...
  return <section className="statement" data-evidence-issue="650">
    ...
    {!scope ? <p>Det valgte regnskabsår har ingen aktiv bogføringsperiode.</p> : <>
      ...
      <div className="row-actions"><label>Vis <select aria-label="Statusfilter" value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value as "" | WorkbenchStatus }))}><option value="">Alle tilstande</option>{Object.entries(stateLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><input aria-label="Søg i bank, bilag, part eller konto" value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} placeholder="Søg på navn eller tekst" /><button type="button" className="btn secondary" onClick={() => { setFilters({ status: "", search: "" }); void refresh(0); }}>Nulstil filtre</button><button type="button" className="btn secondary" onClick={() => setAdvanced((value) => !value)}>Avancerede filtre</button></div>
      {advanced && <details open data-evidence-progressive><summary>Se bilag og kontering</summary><p className="muted">Tekniske identifikatorer vises kun i dokumenteret evidens på den enkelte række.</p></details>}

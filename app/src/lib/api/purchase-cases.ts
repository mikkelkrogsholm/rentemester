...
export const purchaseCasesApi = {
  createPurchaseCase:(slug:string,input:{caseId?:string;source:{kind:"document"|"bank_transaction"|"payable";id:number};documentationOutcome?:"ordinary_evidence_sufficient"|"alternative_evidence_assessed"|"unresolved";note?:string;idempotencyKey:string}) => request<{ok:true;purchaseCase:unknown}>(`/api/companies/${encodeURIComponent(slug)}/purchase-cases`,{method:"POST",headers:{"idempotency-key":input.idempotencyKey},body:JSON.stringify({...input,confirm:true})}),

...
export const purchaseCasesApi = {
  ...
  reviewPurchaseCaseGroup:(slug:string,input:{groupId?:string;members:Array<{caseId:string;expectedVersion:number;expectedSourceFingerprint:string}>;documentationOutcome:"ordinary_evidence_sufficient"|"alternative_evidence_assessed"|"unresolved";note?:string;idempotencyKey:string}) => request<{ok:true;group:unknown}>(`/api/companies/${encodeURIComponent(slug)}/purchase-cases/group-review`,{method:"POST",headers:{"idempotency-key":input.idempotencyKey},body:JSON.stringify({...input,confirm:true})}),
};
